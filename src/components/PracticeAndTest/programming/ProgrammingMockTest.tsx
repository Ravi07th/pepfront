...
import Timer from '../General_aptitude/logicalReasoning/Timer.tsx';
import QuestionPanel from '../General_aptitude/logicalReasoning/QuestionPanel';
import ResultsPage from '../General_aptitude/logicalReasoning/ResultsPage';
import SolutionViewer from '../General_aptitude/logicalReasoning/SolutionViewer';

// Import all programming question data
...
type ProgrammingQuestionStatus = 
  | 'not-visited'
  | 'not-answered'
  | 'current'
  | 'answered'
...
const ProgrammingMockTest: React.FC = () => {
  const { topic } = useParams<{ topic: string }>();
  const navigate = useNavigate();
  const [isFullScreen, setIsFullScreen] = useState(false);
  
  // Create mock topic object based on URL parameter
  const mockTopic: ProgrammingTopic = useMemo(() => ({
    id: topic || 'c-programming',
    name: topic ? topic.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') : 'C Programming',
    description: 'Mock test for programming topic',
  ...
  const questions: ProgrammingQuestion[] = useMemo(() => {
    let topicQuestions: ProgrammingQuestion[] = [];
    
    // Map topic ID to corresponding question data
    switch (topic) {
      case 'comprehensive-programming':
        // Combine questions from all programming topics
        const allQuestions = [
          ...cQuestions,
          ...cppQuestions,
          ...pythonQuestions,
        ...
        // Shuffle and take 60 questions (10 from each topic)
        const shuffled = shuffleArray(allQuestions);
        topicQuestions = shuffled.slice(0, 60);
        break;
      case 'c-programming':
        topicQuestions = cQuestions;
        break;
      case 'cpp-programming':
        topicQuestions = cppQuestions;
        break;
      case 'python':
        topicQuestions = pythonQuestions;
        break;
      case 'javascript':
        topicQuestions = javascriptQuestions;
        break;
      case 'java':
        topicQuestions = javaQuestions;
        break;
      case 'html-css':
         topicQuestions = htmlCssQuestions;
         break;
       default:
         topicQuestions = cQuestions; // Default fallback
     }

    // Convert questions to have proper id format and add topicId
    const formattedQuestions = topicQuestions.map((q, index) => ({
      ...q,
      id: `${topic}-q${index + 1}`,
      topicId: topic
    }));
  ...
  const handleReturnHome = () => {
    ...
    // Navigate back to programming topics
...
export default ProgrammingMockTest;
