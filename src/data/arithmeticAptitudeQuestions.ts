...
// Define total desired number of questions
const TOTAL_QUESTIONS = 5;

// Calculate how many new questions to generate
const questionsToGenerate = TOTAL_QUESTIONS - arithmeticAptitudeQuestions.length;

// Generate remaining questions programmatically in a new array
const generatedQuestions = Array.from({ length: questionsToGenerate }, (_, idx) => {
  const questionNumber = idx + arithmeticAptitudeQuestions.length + 1;
  const options = [
    `${questionNumber * 2}`,
    `${questionNumber * 3}`,
    `${questionNumber * 4}`,
  ...
  return {
    id: `aa-${questionNumber}`,
    question: `Arithmetic Question ${questionNumber}: Calculate the mathematical expression for problem ${questionNumber}?`,
    options,
    correctAnswer: correctIndex,
    explanation: `Explanation for Arithmetic question ${questionNumber}: This involves mathematical calculations and formulas.`,
    difficulty: (['easy', 'medium', 'hard'] as const)[Math.floor(Math.random() * 3)],
  };
});

// Combine initial and generated questions to make final array
export const fullArithmeticAptitudeQuestions = [
  ...arithmeticAptitudeQuestions,
  ...generatedQuestions,
];
